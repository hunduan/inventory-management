...
export default function PurchaseListPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setOrders(items);
      // Auto-expand single-product orders
      ...
      <ScrollView scrollY style={{ flex: 1, padding: 16 }}>
        {loading ? (
          ...
          <View style={{ backgroundColor: '#ffffff', borderRadius: 16, padding: 48, alignItems: 'center' }}>
            ...
            <Text style={{ fontSize: 14, color: '#a8a29e', marginBottom: 8 }}>{search ? '无匹配结果' : '暂无采购记录'}</Text>
            {!search && (
              <View
                  ...
                  <View style={{ borderTop: '1px solid #f5f5f4', paddingTop: 12, paddingBottom: 12, paddingLeft: 16, paddingRight: 16 }}>
                    {items.map((item: any, i: number) => (
                      <View key={i} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingTop: 6, paddingBottom: 6, borderBottom: i < items.length - 1 ? '1px solid #f0f0f0' : 'none' }}>
                        ...
                        <Text style={{ flex: 2, fontSize: 13, color: '#78716c', textAlign: 'center' }}>{item.quantity} x {Number(item.unitCost || 0).toFixed(2)}</Text>
                        <Text style={{ flex: 2, fontSize: 13, fontWeight: 600, color: '#1c1917', textAlign: 'right' }}>{((item.quantity || 0) * Number(item.unitCost || 0)).toFixed(2)}</Text>
                      </View>
                    ))}
