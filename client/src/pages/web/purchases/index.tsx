  ...
  return (
    <AppShell>
      <View className="mb-4 flex gap-2 flex-wrap">
        ...
        <Button className="bg-green-600 text-white px-4 rounded-lg" onClick={() => Taro.navigateTo({ url: '/pages/web/purchases/new' })}>新增采购</Button>
      </View>
      ...
      <View className="bg-white rounded-lg shadow overflow-hidden">
        ...
        {orders.map((o) => (
          <View key={o.id} className="flex p-4 border-b items-center hover:bg-gray-50">
            ...
            <Text className="flex-1">{o.supplier?.name || o.supplierName || '-'}</Text>
            ...
            <View className="w-28 flex gap-1">
              ...
              {o.status === 'CONFIRMED' && (
                <>
                  <Button size="small" className="bg-green-500 text-white" onClick={() => handleAction(o.id, 'receive')}>入库</Button>
                  <Button size="small" className="bg-red-500 text-white" onClick={() => handleAction(o.id, 'cancel')}>取消</Button>
                </>
              )}
