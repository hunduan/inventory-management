...
import type { PurchaseOrder, PaginatedResponse, Warehouse, Product } from '../../types';
...
export default function PurchasesList() {
  ...
  useEffect(() => {
    ...
    // Single initial fetch — if nav params exist, use them
  ...
  const fetchData = async (navDateRange?: { startDate: string; endDate: string }) => {
    ...
    try {
      ...
      params.set('limit', '20');
      const result = await purchasesApi.list(params.toString());
      setData(result);
    } catch (err: unknown) {
  ...
  const columns = [
    ...
    },
    {
      title: '仓库',
      key: 'warehouse',
      render: (_: unknown, record: PurchaseOrder) => record.warehouse?.name || '-',
    },
    {
      title: '金额',
    ...
    <div>
      <Card style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <Button icon={<DownloadOutlined />} onClick={() => purchasesApi.exportExcel()}>导出</Button>
      ...
      {loading ? (
              ...
              expandedRowRender: (record) => (
                <Table
                  ...
                  columns={[
                    ...
                    { title: '已入库', dataIndex: 'receivedQty', key: 'receivedQty', render: (v: number) => v || 0 },
                  ]}
                />
              ),
            }}
        ...
        <Empty description="暂无采购单" style={{ padding: 80 }}>
          <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/purchases/new')}>
            新增采购单
          </Button>
        </Empty>
